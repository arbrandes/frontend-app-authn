...
const RegistrationPage = (props) => {
  ...
  const flags = {
    ...
    autoGeneratedUsernameEnabled: getAppConfig('openedxAuthn').ENABLE_AUTO_GENERATED_USERNAME,
  };
  const {
  ...
  useEffect(() => {
    if (registrationResult.success) {
      ...
      setCookie(getAppConfig('openedxAuthn').USER_RETENTION_COOKIE_NAME, true);
    }
  }, [registrationResult]);
  ...
  const renderForm = () => {
    ...
    return (
      <>
        <Helmet>
          <title>{formatMessage(messages['register.page.title'], { siteName: getConfig().siteName })}</title>
        </Helmet>
        <RedirectLogistration
          host={host}
          ...
          redirectToProgressiveProfilingPage={
            getAppConfig('openedxAuthn').ENABLE_PROGRESSIVE_PROFILING_ON_AUTHN && !!Object.keys(optionalFields.fields).length
          }
        />
  ...
  if (tpaHint) {
    ...
    if (skipHintedLogin) {
      window.location.href = getConfig().lmsBaseUrl + provider.registerUrl;
      return null;
    }
